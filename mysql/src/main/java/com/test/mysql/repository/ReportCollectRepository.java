...
public interface ReportCollectRepository extends JpaRepository<F_garbageCollect, Long> {
     ...
     * @return
     */
    @Query(value = "select NEW com.test.mysql.entity.F_garbageCollect(e.categoryId ,e.categoryName,e.department ,sum(e.netWeight),sum(mweight),max(transitp),max(transitpt)) from f_garbage e " +
