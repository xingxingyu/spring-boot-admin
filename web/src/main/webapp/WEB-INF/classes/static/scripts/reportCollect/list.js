...
function fillData(data) {
    ...
    $.each(data, function (k, v) {
        var html = "";
        html += '<tr> ' +
            '<td>' + (v.department == null ? '泌尿科' : v.department) + '</td>' +
            '<td>' + (v.categoryId == null ? '3' : v.categoryId) + '</td>' +
            '<td>' + (v.categoryName == null ? '化学性废物' : v.categoryName) + '</td>' +
            '<td>' + (v.netWeight == null ? '30' : v.netWeight) + '</td>';
        $list.append($(html));
...
function exportExcel() {
    var u = "./exportExcel?start="+document.getElementById("start").value+"&end="+document.getElementById('end').value;
